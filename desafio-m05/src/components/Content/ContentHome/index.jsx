...
import { clear, getItem } from '../../../functions/storage.jsx';
...
const ContentHome = () => {
  ...
  async function handleApiData() {
    try {
      ...
      const payed = (billingsData.billings).filter(billing => (billing.status == 'Paga'));
      const overdue = (billingsData.billings).filter(billing => (billing.status == 'Vencida'));
      const preview = (billingsData.billings).filter(billing => (billing.status == 'Pendente'));
